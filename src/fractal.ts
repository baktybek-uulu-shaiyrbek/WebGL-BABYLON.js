 ...
 */
class Fractal {
  readonly rootDiameter = 4096;              // диаметр корневой сферы
  readonly ratio = 0.5;                      // во сколько раз мельче следующий уровень
  ...
  readonly maxDepth = 14;
  readonly masks = [4, 2, 1, 6, 3, 5];       // цвета уровней по кругу: 100, 010, 001, 110, 011, 101

  radius(level: number): number {
   ...
   */
  pick(origin: number[], dir: number[], maxLevel: number, visible: boolean[]): number {
    ...
    const enter = (c: number[], l: number): number => {
      ...
      if (b + sb < 0) return -1;
      if (Math.max(b - sb, 0) >= best) return -1;
